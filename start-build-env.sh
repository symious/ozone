...
docker build -t ozone-build -f $DOCKER_FILE $DOCKER_DIR

USER_NAME=jenkins1000
USER_ID=1000
GROUP_ID=1000
...
docker build -t "ozone-build-${USER_ID}" - <<UserSpecificDocker
FROM ozone-build
RUN sudo rm -f /var/log/faillog /var/log/lastlog
#RUN groupadd --non-unique -g ${GROUP_ID} ${USER_NAME}
#RUN useradd -g ${GROUP_ID} -u ${USER_ID} -k /root -m ${USER_NAME} -d "${DOCKER_HOME_DIR}"
#RUN echo "${USER_NAME} ALL=NOPASSWD: ALL" > "/etc/sudoers.d/ozone-build-${USER_ID}"
RUN groupadd --non-unique -g ${DOCKER_GROUP_ID} docker
RUN usermod -a -G ${GROUP_ID},${DOCKER_GROUP_ID} -d "${DOCKER_HOME_DIR}" ${USER_NAME}
...
docker run --rm=true $DOCKER_INTERACTIVE_RUN \
  -v "${PWD}:${DOCKER_HOME_DIR}/ozone${V_OPTS:-}" \
  -w "${DOCKER_HOME_DIR}/ozone" \
